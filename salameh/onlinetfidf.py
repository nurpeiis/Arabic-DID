...
From: https://github.com/idoshlomo/online_vectorizers/blob/master/online_vectorizers.py
"""
import logging
...
class OnlineCountVectorizer(CountVectorizer):
    ...
    def partial_refit_transform(self, raw_documents):
        ...
        if oov_tokens:
            ...
            oov_vocabulary = dict(zip(oov_tokens, list(
                range(max_index + 1, max_index + 1 + len(oov_tokens), 1))))
...
class OnlineTfidfVectorizer(OnlineCountVectorizer):
    ...
    def partial_refit_transform(self, raw_documents):
        ...
        X : array, [n_samples, n_features]
            Document-term matrix.
        """
        logger.info("validate: checking {} records for new tokens".format(
            len(raw_documents)))
        current_vocabulary_size = len(self.vocabulary_)
        X = super().partial_refit_transform(raw_documents)
        vocabulary_size_change = len(
            self.vocabulary_) - current_vocabulary_size
    ...
    def _update_idf(self):
        ...
        n_features.
        """
        self._tfidf.idf = np.log(
            float(self.n_samples) / (self.df + int(self._tfidf.smooth_idf))) + 1.0
